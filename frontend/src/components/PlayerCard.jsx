    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="flex items-center gap-1.5 truncate text-sm font-semibold text-ink">
          ...
          {isMe && <span className="text-xs font-normal text-ink-faint">(you)</span>}
        </p>
        <div className="flex items-center gap-1.5 text-xs text-ink-faint">
          <span>{propertyCount} {propertyCount === 1 ? 'property' : 'properties'}</span>
          {player.inJail && <span className="rounded bg-rent/15 px-1.5 py-0.5 font-medium text-rent">In Jail</span>}
        </div>
      </div>

      <div className="shrink-0 text-right">
        {player.bankrupt ? (
          <span className="text-xs font-bold uppercase tracking-wide text-rent">Bankrupt</span>
        ) : (
