...
function PlayerTokens({ players }) {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-20">
      {active.map((player) => {
        const { xPct, yPct } = tokenTranslatePercent(player.position);
        ...
        return (
          <div
            key={player.id}
            className="absolute left-0 top-0 h-0 w-0 transition-transform duration-700 ease-[cubic-bezier(0.34,1.56,0.64,1)]"
            style={{
              // translate() with mixed % and px in one expression via
              // calc(): the % component (continuous, animates smoothly)
              // moves the token to its tile's center; the px component
              // (the small clustering offset) rides along additively.
              transform: `translate(calc(${xPct}% + ${offset.x}px), calc(${yPct}% + ${offset.y}px))`,
            }}
          >
            <span
              className="block -translate-x-1/2 -translate-y-1/2 h-4 w-4 rounded-full ring-[2.5px] ring-white shadow-token sm:h-[18px] sm:w-[18px]"
