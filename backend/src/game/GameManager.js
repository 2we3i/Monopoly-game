...
class GameManager {
  ...
  rollDice(playerId) {
    ...
    // Cleared up front so a roll that doesn't actually move anyone (e.g.
    // failing to roll doubles in Jail) doesn't leave a stale lastMove from
    // an earlier turn sitting around to be incorrectly replayed.
    state.lastMove = null;
  ...
  payJailFine(playerId) {
    ...
    player.inJail = false;
    player.jailTurns = 0;
    state.lastMove = null; // stays on the Jail tile until their next roll
  ...
  _sendToJail(player) {
    ...
    player.consecutiveDoubles = 0;
    // Jail sends are an instant relocation, not a walk - the real game
    // doesn't animate this as stepping tile-by-tile back to Jail.
    state.lastMove = {
      moveId: `${player.id}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      playerId: player.id,
      path: [JAIL_TILE],
      teleport: true,
    };
  ...
  // Builds the ordered list of intermediate tile ids a token should
  // visually step through to go from oldPos to newPos, walking the short
  // way around in the given direction. Used only to populate state.lastMove
  // for client-side animation - never consulted for any game-logic
  // decision, so a bug here can only ever affect how a move *looks*, not
  // whether it's legal or what it does.
  static _buildWalkPath(oldPos, newPos, direction) {
    const path = [];
    let cursor = oldPos;
    // Safety cap at 40 iterations (one full lap) so a logic error here can
    // never hang the loop instead of just producing a wrong-looking path.
    for (let i = 0; i < 40 && cursor !== newPos; i++) {
      cursor = direction > 0 ? (cursor + 1) % 40 : (cursor - 1 + 40) % 40;
      path.push(cursor);
    }
    return path;
  }
  ...
  _movePlayer(player, steps, opts = {}) {
    ...
    state.lastMove = {
      // Unique per move so the client can dedupe by VALUE. lastMove is
      // re-created from JSON on every socket broadcast, so a client-side
      // check like `moveDescriptor !== previousMoveDescriptor` compares
      // object references that are never equal after a round-trip through
      // JSON, even when the move itself hasn't changed - that mismatch is
      // what caused the token animation to replay on unrelated broadcasts.
      moveId: `${player.id}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      playerId: player.id,
      path: GameManager._buildWalkPath(oldPos, newPos, steps >= 0 ? 1 : -1),
      teleport: false,
    };
    if (passedGo) {
  ...
  _moveToTile(player, tileId, opts = {}) {
    ...
    // Card-driven "advance to X" moves always walk forward (matching the
    // real game's "advance to" phrasing), even when the destination id is
    // numerically smaller than the origin (that's exactly the passedGo
    // case - forward, wrapping past GO).
    state.lastMove = {
      moveId: `${player.id}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      playerId: player.id,
      path: GameManager._buildWalkPath(oldPos, tileId, 1),
      teleport: false,
    };
    if (passedGo) {
      player.money += GO_SALARY;
  ...
  _drawCard(player, deckName) {
    ...
    state.addLog(`${player.nickname} drew: ${card.text.en}`, 'DREW_CARD', { name: player.nickname, cardText: card.text, playerId: player.id, deckName });
