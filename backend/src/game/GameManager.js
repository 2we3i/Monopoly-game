...
class GameManager {
  ...
  addPlayer({ playerId, socketId, nickname }) {
    ...
    state.addLog(`${player.nickname} joined the room.`);
    return player;
  }
  ...
  removePlayer(playerId) {
    ...
    if (state.phase === 'LOBBY') {
      ...
      state.addLog(`${player.nickname} left the room.`);
    } else {
  ...
  markDisconnected(playerId) {
    ...
    if (player) {
      player.connected = false;
      player.socketId = null;
      this.state.addLog(`${player.nickname} disconnected.`);
    }
  }
  ...
  reconnectPlayer(playerId, socketId) {
    ...
    this.state.addLog(`${player.nickname} reconnected.`);
    return player;
  }
  ...
  startGame(requesterId) {
    ...
    state.addLog('The game has started! Good luck.');
  }
  ...
  advanceTurn() {
    ...
    if (cp) state.addLog(`It's ${cp.nickname}'s turn.`);
  ...
  checkWinCondition() {
    ...
    if (active.length === 1 && state.players.length > 1) {
      ...
      state.addLog(`${active[0].nickname} wins the game!`);
    }
  }
  ...
  rollDice(playerId) {
    ...
    if (isDouble) {
      ...
      if (player.consecutiveDoubles >= 3) {
        ...
        state.addLog(`${player.nickname} rolled doubles three times in a row and was sent to Jail!`);
        state.phase = 'ACTION';
  ...
  _handleJailRoll(player, d1, d2, isDouble) {
    ...
    if (isDouble) {
      player.inJail = false;
      player.jailTurns = 0;
      state.addLog(`${player.nickname} rolled doubles and got out of Jail!`);
      this._movePlayer(player, d1 + d2);
      state.phase = 'ACTION';
    ...
    if (player.jailTurns >= MAX_JAIL_TURNS) {
      ...
      player.inJail = false;
      player.jailTurns = 0;
      state.addLog(`${player.nickname} paid $${JAIL_FINE} after 3 failed attempts and left Jail.`);
      this._movePlayer(player, d1 + d2);
      state.phase = 'ACTION';
      return { paidFineAndMoved: true };
    }
    state.addLog(`${player.nickname} failed to roll doubles in Jail (attempt ${player.jailTurns}/${MAX_JAIL_TURNS}).`);
    state.phase = 'ACTION';
  ...
  payJailFine(playerId) {
    ...
    player.inJail = false;
    player.jailTurns = 0;
    state.addLog(`${player.nickname} paid $${JAIL_FINE} to get out of Jail.`);
  }
  ...
  useGetOutOfJailCard(playerId) {
    ...
    player.inJail = false;
    player.jailTurns = 0;
    state.addLog(`${player.nickname} used a Get Out of Jail Free card.`);
  }
  ...
  _movePlayer(player, steps, opts = {}) {
    ...
    if (passedGo) {
      ...
      state.addLog(`${player.nickname} passed Start and collected $${GO_SALARY}.`);
    }
  ...
  _resolveTile(player, opts = {}) {
    ...
    switch (tile.type) {
      ...
      case TILE_TYPES.TAX:
        ...
        state.addLog(`${player.nickname} paid $${tile.amount} in ${tile.name}.`);
        break;
      case TILE_TYPES.GO_TO_JAIL:
        this._sendToJail(player);
        state.addLog(`${player.nickname} was sent to Jail!`);
        break;
      ...
      case TILE_TYPES.FREE_PARKING:
        if (state.freeParkingPot > 0) {
          ...
          state.addLog(`${player.nickname} landed on Free Parking and collected $${state.freeParkingPot}!`);
  ...
  _payRent(player, tile, propState, doubleRent = false) {
    ...
    player.money -= rent;
    owner.money += rent;
    state.addLog(`${player.nickname} paid $${rent} rent to ${owner.nickname} for ${tile.name}.`);
  }
  ...
  _drawCard(player, deckName) {
    ...
    state.addLog(`${player.nickname} drew: ${card.text}`);
  ...
  buyProperty(playerId, tileId) {
    ...
    state.pendingAction = null;
    state.addLog(`${player.nickname} bought ${tile.name} for $${tile.price}.`);
  }
  ...
  declinePurchase(playerId, tileId) {
    ...
    const tile = BOARD[tileId];
    state.addLog(`${state.currentPlayer.nickname} declined to buy ${tile.name}.`);
    state.pendingAction = null;
  ...
  mortgageProperty(playerId, tileId) {
    ...
    state.addLog(`${player.nickname} mortgaged ${tile.name}.`);
  }
  ...
  unmortgageProperty(playerId, tileId) {
    ...
    state.addLog(`${player.nickname} paid off the mortgage on ${tile.name}.`);
  }
  ...
  buildHouse(playerId, tileId) {
    ...
    propState.houses += 1;
    state.addLog(`${player.nickname} built ${propState.houses === 5 ? 'a hotel' : `house #${propState.houses}`} on ${tile.name}.`);
  }
  ...
  sellHouse(playerId, tileId) {
    ...
    state.addLog(`${player.nickname} sold a house on ${tile.name}.`);
  }
  ...
  endTurn(playerId) {
    ...
    if (state.lastRoll && state.lastRoll.isDouble && !player.inJail && player.consecutiveDoubles > 0) {
      state.phase = 'ROLLING';
      state.lastRoll = null;
      state.addLog(`${player.nickname} rolled doubles and goes again!`);
      return;
    }
  ...
  declareBankruptcy(playerId) {
    ...
    player.money = 0;
    state.addLog(`${player.nickname} declared bankruptcy and is out of the game.`);
